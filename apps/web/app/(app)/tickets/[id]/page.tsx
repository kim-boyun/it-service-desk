...
export default function TicketDetailPage() {
  ...
  const updateStatusM = useMutation({
    ...
    onSuccess: () => {
      ...
      setNote("");
      if (status === "resolved") router.push("/tickets/resolved");
      if (status === "closed") router.push("/tickets/review");
    },
  });
  ...
  const addCommentM = useMutation({
    mutationFn: () =>
      api(`/tickets/${ticketId}/comments`, {
        method: "POST",
        body: { body: commentBody, is_internal: isStaffScope ? commentInternal : false },
      }),
    onSuccess: () => {
      markLocalRead(ticketId);
      setCommentBody("");
  ...
  const t = data.ticket;
  const canEdit = !isStaffScope && t.requester_id === me.id && t.status === "open";
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {canEdit && (
            <button
              className="border rounded px-3 py-2 text-sm bg-white text-red-600 hover:bg-red-50"
              onClick={() => {
          ...
          {canEdit && (
            <button
              className="border rounded px-3 py-2 text-sm bg-white text-gray-800 hover:bg-gray-50"
              onClick={() => {
