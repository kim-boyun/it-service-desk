...
export default function DraftTicketEditPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["draft-ticket", draftId],
    queryFn: () => api<DraftTicket>(`/draft-tickets/${draftId}`),
  ...
  const updateDraft = useMutation({
    mutationFn: async (includeCategory: boolean) => {
      ...
      return await api(`/draft-tickets/${draftId}`, {
        method: "PATCH",
        body: payload,
      });
    },
  ...
  const publishDraft = useMutation({
    mutationFn: async () => {
      const created = await api<TicketOut>(`/draft-tickets/${draftId}/publish`, { method: "POST" });
  ...
  const deleteDraft = useMutation({
    mutationFn: async () => api(`/draft-tickets/${draftId}`, { method: "DELETE" }),
    onSuccess: () => {
      setIsDirty(false);
      router.replace("/tickets/drafts");
    },
    onError: (err: any) => {
  ...
  return (
    <div className="p-6 w-full max-w-none">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4 rounded-2xl border border-blue-gray-100 bg-white/80 shadow-sm backdrop-blur px-5 py-4">
        <h1 className="text-2xl font-semibold">임시저장 요청 수정</h1>
      </div>
      <form onSubmit={onPublish} className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          >
            {publishDraft.isPending ? "등록 중..." : "등록"}
          </button>
          <button
            type="button"
            className="border rounded px-4 py-2 text-sm bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-60"
            onClick={onSaveDraft}
            disabled={updateDraft.isPending}
          >
            {updateDraft.isPending ? "임시저장 중..." : "임시저장"}
          </button>
          <button
            type="button"
